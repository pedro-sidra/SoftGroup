...
class ScanNetEval(object):
    ...
    def evaluate_matches(self, matches):
        ...
        # results: class x iou
        ap = np.zeros(
            (len(dist_threshes), len(self.eval_class_labels), len(ious)), np.float
        )
        rc = np.zeros(
            (len(dist_threshes), len(self.eval_class_labels), len(ious)), np.float
        )
    ...
    def compute_averages(self, aps, rcs):
        ...
        for li, label_name in enumerate(self.eval_class_labels):
            ...
            avg_dict["classes"][label_name]["rc"] = np.average(
                rcs[d_inf, li, oAllBut25]
            )
    ...
    def assign_instances_for_scan(self, preds, gts):
        ...
        for pred in preds:
            ...
            pred_instance["filename"] = "{}_{}".format(
                pred["scan_id"], num_pred_instances
            )  # dummy
    ...
    def write_result_file(self, avgs, filename):
        ...
        with open(filename, "w") as f:
            ...
            for class_name in self.eval_class_labels:
                ...
                f.write(
                    _SPLITTER.join([str(x) for x in [class_name, ap, ap50, ap25]])
                    + "\n"
                )
