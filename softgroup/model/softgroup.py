...
class SoftGroup(nn.Module):
    def __init__(
        ...
        self.semantic_linear = MLP(
            channels, semantic_classes, norm_fn=norm_fn, num_layers=2
        )
        self.offset_linear = MLP(channels, 3, norm_fn=norm_fn, num_layers=2)

        # topdown refinement path
        if not semantic_only:
            self.tiny_unet = UBlock(
                [channels, 2 * channels], norm_fn, 2, block, indice_key_id=11
            )
            self.tiny_unet_outputlayer = spconv.SparseSequential(
                norm_fn(channels), nn.ReLU()
            )
            self.cls_linear = nn.Linear(channels, instance_classes + 1)
            self.mask_linear = MLP(
                channels, instance_classes + 1, norm_fn=None, num_layers=2
            )
        ...
        if not self.semantic_only:
            ...
            if proposals_offset.shape[0] > self.train_cfg.max_proposal_num:
                proposals_offset = proposals_offset[
                    : self.train_cfg.max_proposal_num + 1
                ]
            ...
            (
                ...
                iou_scores,
                mask_scores,
            ) = self.forward_instance(inst_feats, inst_map)
    ...
    def point_wise_loss(
        ...
        if self.semantic_weight:
            weight = torch.tensor(
                self.semantic_weight, dtype=torch.float, device="cuda"
            )
        else:
            weight = None
        ...
        if pos_inds.sum() == 0:
            ...
            offset_loss = (
                F.l1_loss(
                    pt_offsets[pos_inds], pt_offset_labels[pos_inds], reduction="sum"
                )
                / pos_inds.sum()
            )
    ...
    def instance_loss(
        ...
        assigned_gt_inds = fg_ious_on_cluster.new_full(
            (num_proposals,), -1, dtype=torch.long
        )
        ...
        slice_inds = torch.arange(
            0, labels.size(0), dtype=torch.long, device=labels.device
        )
        ...
        iou_score_loss = (iou_score_loss * iou_score_weight).sum() / (
            iou_score_weight.sum() + 1
        )
    ...
    def forward_test(
        ...
        **kwargs,
        ...
        if (
            "semantic" in self.test_cfg.eval_tasks
            or "panoptic" in self.test_cfg.eval_tasks
        ):
            ret.update(
                dict(
        ...
        if not self.semantic_only:
            if (
                "instance" in self.test_cfg.eval_tasks
                or "panoptic" in self.test_cfg.eval_tasks
            ):
                if lvl_fusion:
                ...
                _, cls_scores, iou_scores, mask_scores = self.forward_instance(
                    inst_feats, inst_map
                )
            ...
            if "instance" in self.test_cfg.eval_tasks:
                ...
                ret.update(
                    dict(pred_instances=pred_instances, gt_instances=gt_instances)
                )
            if "panoptic" in self.test_cfg.eval_tasks:
                panoptic_preds = self.panoptic_fusion(
                    semantic_preds.cpu().numpy(), pred_instances
                )
    ...
    def forward_grouping(
        ...
        for class_id in range(self.semantic_classes):
            ...
            if len(proposals_offset_list) > 0:
                proposals_idx[:, 0] += (
                    sum([x.size(0) for x in proposals_offset_list]) - 1
                )
    ...
    def pyramid_map(
        self, coords_float, pt_offsets, batch_idxs, level=1, base_size=0.02
    ):
        coords = (coords_float / (base_size * level)).long()
        coords = torch.cat([batch_idxs[:, None], coords], dim=1)
        coords, l2p_map, p2l_map = voxelization_idx(
            coords.cpu(), batch_idxs[-1].item() + 1
        )
    ...
    def pyramid_inverse_map(self, proposals_idx, proposals_offset, num_points, l2p_map):
        proposals = torch.zeros(
            (proposals_offset.size(0) - 1, num_points), dtype=torch.int
        )
    ...
    def get_instances(
        ...
        for i in range(self.instance_classes):
            if i in self.sem2ins_classes:
                ...
                score_pred = cls_scores.new_tensor([1.0], dtype=torch.float32)
                mask_pred = (semantic_pred == i)[None, :].int()
                if lvl_fusion:
                    mask_pred = mask_pred[:, v2p_map.long()]
            else:
                cls_pred = cls_scores.new_full(
                    (num_instances,), i + 1, dtype=torch.long
                )
                ...
                mask_pred = torch.zeros(
                    (num_instances, num_points), dtype=torch.int, device="cuda"
                )
    ...
    def clusters_voxelization(
        ...
        if clusters_idx.size(0) == 0:
            ...
            coords = torch.tensor(
                [
                    [0, 0, 0, 0],
                    [0, spatial_shape - 1, spatial_shape - 1, spatial_shape - 1],
                ],
                dtype=torch.int,
                device="cuda",
            )
            ...
            voxelization_feats = spconv.SparseConvTensor(
                feats, coords, [spatial_shape] * 3, 1
            )
        ...
        clusters_scale = (
            1 / ((coords_max - coords_min) / spatial_shape).max(1)[0] - 0.01
        )
        ...
        if rand_quantize:
            ...
            coords_min -= (
                torch.clamp(spatial_shape - range - 0.001, min=0) * torch.rand(3).cuda()
            )
            coords_min -= (
                torch.clamp(spatial_shape - range + 0.001, max=0) * torch.rand(3).cuda()
            )
        ...
        out_coords, inp_map, out_map = voxelization_idx(
            coords, int(clusters_idx[-1, 0]) + 1
        )
