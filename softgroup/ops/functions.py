...
def ball_query(
    coords, batch_idxs, batch_offsets, radius, mean_active, with_octree=False
):
    if with_octree:
...
def octree_ball_query(coords, mean_active, radius):
    ...
    ops.build_and_export_octree(
        coords, xyzwhl, boxes, pt_inds, pt_start_len, num_levels
    )
    boxes = boxes.cuda()
    ...
    while True:
        ...
        n_totals = ops.octree_ball_query(
            coords,
            boxes,
            pt_inds,
            pt_start_len,
...
class BFSCluster(Function):
    @staticmethod
    def forward(
        ctx, cluster_numpoint_mean, ball_query_idxs, start_len, threshold, class_id
    ):
        """
        :param ctx:
...
class GlobalAvgPool(Function):
    ...
    def backward(ctx, d_output_feats):
        ...
        ops.global_avg_pool_bp(
            d_feats, proposals_offset, d_output_feats.contiguous(), nProposal, C
        )
