...
def validate(epoch, model, val_loader, cfg, logger, writer):
    ...
    progress_bar = tqdm(
        total=len(val_loader) * world_size, disable=not is_main_process()
    )
    ...
    if is_main_process():
        ...
        if "panoptic" in eval_tasks:
            ...
            panoptic_eval = PanopticEval(
                val_set.THING, val_set.STUFF, min_points=eval_min_npoint
            )
            eval_res = panoptic_eval.evaluate(
                all_panoptic_preds, all_sem_labels, all_inst_labels
            )
            writer.add_scalar("val/PQ", eval_res[0], epoch)
            logger.info("PQ: {:.1f}".format(eval_res[0]))
        if "semantic" in eval_tasks:
            logger.info("Evaluate semantic segmentation and offset MAE")
            miou = evaluate_semantic_miou(
                all_sem_preds, all_sem_labels, cfg.model.ignore_label, logger
            )
...
def main():
    ...
    val_loader = build_dataloader(
        val_set, training=False, dist=args.dist, **cfg.dataloader.test
    )
