...
def save_panoptic(root, name, scan_ids, arrs, learning_map_inv, num_classes):
    ...
    os.makedirs(root, exist_ok=True)
    paths = [
        osp.join(root, f"{i}.label".replace("velodyne", "predictions"))
        for i in scan_ids
    ]
    ...
    pool = mp.Pool()
    pool.starmap(
        save_panoptic_single, zip(paths, arrs, learning_map_invs, num_classes_list)
    )
...
def main():
    ...
    dataloader = build_dataloader(
        dataset, training=False, dist=args.dist, **cfg.dataloader.test
    )
    results = []
    ...
    progress_bar = tqdm(
        total=len(dataloader) * world_size, disable=not is_main_process()
    )
    ...
    if is_main_process():
        ...
        if "panoptic" in eval_tasks:
            ...
            panoptic_eval = PanopticEval(
                dataset.THING, dataset.STUFF, min_points=eval_min_npoint
            )
        ...
        if "semantic" in eval_tasks:
            ...
            evaluate_offset_mae(
                offset_preds, offset_labels, inst_labels, ignore_label, logger
            )
