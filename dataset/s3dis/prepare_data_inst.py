...
def get_parser():
    ...
    parser.add_argument(
        "--align", action="store_true", help="processing aligned dataset or not"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="show processing room name or not"
    )
    ...
    for area_id in area_list:
        ...
        for room_name in room_name_list:
            ...
            (
                ...
                instance_labels,
                room_label,
            ) = read_s3dis_format(area_id, room_name, data_root)
            rgb = (rgb / 127.5) - 1
            torch.save(
                (xyz, rgb, semantic_labels, instance_labels, room_label, scene),
                save_path,
            )
