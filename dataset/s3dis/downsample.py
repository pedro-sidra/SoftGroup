...
def get_parser():
    ...
    parser.add_argument(
        "--ratio", type=float, default=0.25, help="random downsample ratio"
    )
    parser.add_argument(
        "--voxel-size",
        type=float,
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="show partition information or not"
    )
...
if __name__ == "__main__":
    ...
    for data_file in glob.glob(osp.join(data_dir, "*.pth")):
        ...
        torch.save(
            (coords, colors, semantic_labels, instance_labels, room_label, scene),
            save_path,
        )
