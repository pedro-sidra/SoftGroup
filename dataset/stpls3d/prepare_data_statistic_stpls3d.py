...
for file in files:
    ...
    points = np.concatenate(
        [
            coords,
            colors,
            sem_labels[:, None].astype(int),
            instance_labels[:, None].astype(int),
        ],
        axis=1,
    )
    for semanticID in semanticIDs:
        ...
        for uniqueInstance in uniqueInstances:
            eachInstance = singleSemantic[
                np.where(singleSemantic[:, 7] == uniqueInstance)
            ]
...
for semanticID in semanticIDs:
    class_numpoint_mean_list.append(
        sum(class_numpoint_mean_dict[semanticID])
        * 1.0
        / len(class_numpoint_mean_dict[semanticID])
    )
...
print(
    [1.0] + [float("{0:0.0f}".format(i)) for i in class_numpoint_mean_list][1:], sep=","
)
...
print(
    [1.0, 1.0] + [float("{0:0.2f}".format(i)) for i in num_points_semantic][2:], sep=""
)
