            ...
            for blockNum, block in enumerate(blocks):
                if len(block) > 10000:
                    outFilePath = os.path.join(
                        outPutFolder, name + str(blockNum) + "_inst_nostuff.pth"
                    )
                    ...
                    if split != "test":
                        ...
                        disableInstanceBySemantic_labels = np.ascontiguousarray(
                            block[:, -2]
                        )
                        disableInstanceBySemantic_labels = (
                            disableInstanceBySemantic_labels.astype(np.int32)
                        )
                        disableInstanceBySemantic_labels = (
                        ...
                        instance_labels = np.where(
                            disableInstanceBySemantic_labels == -100,
                            -100,
                            instance_labels,
                        )
                        ...
                        uniqueInstances = (np.unique(instance_labels))[1:].astype(
                            np.int32
                        )
                        ...
                        instance_labels = remapper_instance[
                        ...
                        if split == "train" and (
                            len(uniqueInstances) < 10
                            or (len(uniqueSemantics) >= (len(uniqueInstances) - 2))
                        ):
